...
//        btn?.titleLabel?.font = UIFont(name:"Noto Sans CJK TC", size:16);
        btn?.setTitleColor(Constants.getDefaultTextColor(), for: .disabled)
        btn?.setTitleColor(Constants.getYellowColor(), for: .selected)
        
        
        if (index == 0) {
            btn?.titleEdgeInsets = UIEdgeInsets(top: 0, left: -33, bottom: 0, right: 0);
        }
        if (index == 1) {
...
//            btn?.titleEdgeInsets = UIEdgeInsets(top: 0, left: -0.8, bottom: 0, right: 0);
            ...
            btn?.titleEdgeInsets = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: -33);
        }
        return item
    }
